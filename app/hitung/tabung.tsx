...
export default function TabungScreen() {
  ...
  const hitungVolume = () => {
    ...
    if (!isNaN(r) && !isNaN(t)) {
      setHasil(Math.PI * Math.pow(r, 2) * t);
    } else {
      setHasil(null);
    }
    Keyboard.dismiss();
  ...
  return (
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <ThemedView style={styles.container}>
        <Stack.Screen options={{ title: 'Isi Tabung' }} />
        
        <ThemedText type="title" style={styles.header}>Isi Tabung</ThemedText>
        ...
        <TouchableOpacity style={styles.button} onPress={hitungVolume}>
          <ThemedText style={styles.buttonText} lightColor="#fff" darkColor="#fff">Hitung</ThemedText>
        </TouchableOpacity>

        {hasil !== null && (
          <ThemedView style={styles.resultContainer}>
            <ThemedText type="subtitle">Hasil (Volume):</ThemedText>
