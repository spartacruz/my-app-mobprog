...
export default function DiskonScreen() {
    ...
    const hitungDiskon = () => {
        ...
        // Tentukan kategori untuk Switch-Case (karena switch tidak bisa langsung pakai range)
        let kategori: number;
        if (h > 1500000)      kategori = 3;
        else if (h >= 1000000) kategori = 2;
        else if (h >= 500000)  kategori = 1;
        else                   kategori = 0;

        let persenDiskon = 0;
        let keterangan = '';

        // Switch-Case menggantikan Nested IF dari soal nomor 2
        switch (kategori) {
            case 3:
                persenDiskon = 30;
    ...
    return (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <ThemedView style={styles.container}>
                <Stack.Screen options={{ title: 'Diskon Switch-Case' }} />
                <ScrollView showsVerticalScrollIndicator={false}>
                    <ThemedText type="title" style={styles.header}>Switch-Case Diskon</ThemedText>
                    <ThemedText style={styles.description}>
                        Soal No.2 diimplementasikan ulang menggunakan Switch-Case
                    </ThemedText>
                    ...
                    {hasil !== null && (
                        <ThemedView style={[styles.resultContainer, { borderColor: diskonColor[hasil.persenDiskon] }]}>
                            ...
                            {/* Label yang menunjukkan Switch-Case dipakai */}
                            <ThemedText style={styles.switchInfo}>
                                switch(kategori) → case {hasil.persenDiskon === 30 ? 3 : hasil.persenDiskon === 20 ? 2 : hasil.persenDiskon === 10 ? 1 : 0}: diskon {hasil.persenDiskon}%
                            </ThemedText>
                    ...
                    <ThemedView style={styles.infoBox}>
                        <ThemedText type="defaultSemiBold" style={styles.infoTitle}>📋 Ketentuan Diskon (Switch-Case)</ThemedText>
                        {[
                            { range: '> Rp 1.500.000',              diskon: '30%', case: 'case 3', color: '#22c55e' },
                            { range: 'Rp 1.000.000 – Rp 1.500.000', diskon: '20%', case: 'case 2', color: '#3b82f6' },
                            { range: 'Rp 500.000 – Rp 999.999',     diskon: '10%', case: 'case 1', color: '#f59e0b' },
                            { range: '< Rp 500.000',                 diskon: '0%',  case: 'default', color: '#9ca3af' },
                        ].map((item, i) => (
                            <View key={i} style={styles.infoRow}>
                                <ThemedText style={styles.infoCase}>{item.case}</ThemedText>
...
const styles = StyleSheet.create({
    ...
    diskonBadge: { textAlign: 'center', marginBottom: 6 },
    switchInfo: { textAlign: 'center', fontSize: 11, opacity: 0.4, fontFamily: 'monospace', marginBottom: 12 },
    ...
    infoCase: { width: 60, fontSize: 11, opacity: 0.45, fontFamily: 'monospace' },
    infoRange: { flex: 1, opacity: 0.8, fontSize: 12 },
    infoDiskon: { width: 40, textAlign: 'right', fontWeight: 'bold' },
});
